...
const formInicial = { nome: '', tipo: 'hard', icone: '' }
...
function Skills() {
  ...
  const selecionar = (id) => {
    ...
    if (modo === 'edit') {
      ...
      if (s) setForm({ nome: s.nome, tipo: s.tipo, icone: s.icone })
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modo === 'add') {
        const nova = await adicionarSkill(form)
        setSkills(prev => [...prev, nova])
      } else if (modo === 'edit' && idSelecionado) {
        const atualizada = await editarSkill(Number(idSelecionado), form)
        setSkills(prev => prev.map(s => s.id === atualizada.id ? atualizada : s))
      } else if (modo === 'delete' && idSelecionado) {
        await excluirSkill(Number(idSelecionado))
        setSkills(prev => prev.filter(s => s.id !== Number(idSelecionado)))
      }
      fechar()
    } catch {
  ...
  const tituloModal = modo === 'add' ? 'Adicionar Habilidade' : modo === 'edit' ? 'Editar Habilidade' : 'Excluir Habilidade'
          ...
          <div className={styles.contentCardHard}>
            {hardSkills.map(skill => (
              <div key={skill.id} className={styles.itemHard}>
                {skill.icone && <img className={styles.imgItemHard} src={skill.icone} alt={skill.nome} />}
                <h3>{skill.nome}</h3>
              </div>
            ))}
          </div>
          ...
          {(modo === 'edit' || modo === 'delete') && (
            <div className={modalStyles.field}>
              ...
              <select className={modalStyles.select} value={idSelecionado} onChange={(e) => selecionar(e.target.value)} required>
                ...
                {skills.map(s => <option key={s.id} value={s.id}>[{s.tipo}] {s.nome}</option>)}
              </select>
            </div>
          )}
          ...
          {mostrarFormulario && (
            <>
              ...
              <div className={modalStyles.field}>
                ...
                <select className={modalStyles.select} value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value, icone: '' })}>
              ...
              {form.tipo === 'hard' ? (
                <div className={modalStyles.field}>
                  ...
                  <input className={modalStyles.input} type="url" placeholder="https://..." value={form.icone} onChange={(e) => setForm({ ...form, icone: e.target.value })} />
                </div>
              ) : (
                <div className={modalStyles.field}>
                  <label className={modalStyles.label}>Ícone</label>
                  <select className={modalStyles.select} value={form.icone} onChange={(e) => setForm({ ...form, icone: e.target.value })} required>
                    <option value="">-- Escolha um ícone --</option>
                    {ICONES_SOFT.map(nome => <option key={nome} value={nome}>{nome}</option>)}
                  </select>
                </div>
              )}
            </>
